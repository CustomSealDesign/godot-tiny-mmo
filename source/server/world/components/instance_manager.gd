...
## portal room. Login spawn is woodland (see _on_peer_connected), not this hub.
const RECALL_INSTANCE_NAME: String = "GuildHouse"
## Legacy social hub instance name (tavern / guild house). Kept for recall and
## related systems; login no longer routes here (3D client needs woodland).
...
func _on_peer_connected(peer_id: int) -> void:
	...
	# Normal login (first-ever and returning): land in woodland — the 3D Xianxia starter map
	# with SpiritTree / spawn floor. JailList-jailed accounts already returned above.
	# JAIL / TAVERN / RECALL constants stay for jail teleport, recall, and related systems;
	# they are no longer the default login destination (those maps are legacy 2D).
	var target_res: InstanceResource = instance_collection.get("woodland", null)
	if target_res != null:
	...
	# Fallback: woodland is missing or mid-load — land in the default instance so
