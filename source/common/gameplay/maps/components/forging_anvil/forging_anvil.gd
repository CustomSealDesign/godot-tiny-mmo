class_name ForgingAnvil
extends Node2D
## A 3D forging anvil the local player can click to pathfind toward and craft at.
## Server registers each instance on its Map by node name; the client paths to the
## nearest empty adjacent tile and fires forging.start only after arrival.


const INTERACT_RANGE: float = ForgingService.INTERACT_RANGE
const MARKER_SCENE: PackedScene = preload("res://source/common/gameplay/maps/components/interactable_marker.tscn")

@export var recipe_id: StringName = RecipeDatabase.RECIPE_COPPER_SWORD
...
func _ready() -> void:
	if multiplayer.is_server():
		...
		if map != null:
			map.register_keyed(map.forging_anvils, StringName(name), self, "forging anvil")
		return
...
func _request_start() -> void:
	...
	Client.request_data(
		&"forging.start",
		Callable(),
		{"anvil": String(name), "recipe": String(recipe_id)},
