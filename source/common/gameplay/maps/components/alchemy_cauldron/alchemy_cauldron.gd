...
extends Node2D
## A 3D alchemy cauldron the local player can click to pathfind toward and craft at.
## Server registers each instance on its Map by node name; the client paths to the
## nearest empty adjacent tile and fires alchemy.start only after arrival.


const INTERACT_RANGE: float = AlchemyService.INTERACT_RANGE
const MARKER_SCENE: PackedScene = preload("res://source/common/gameplay/maps/components/interactable_marker.tscn")

@export var recipe_id: StringName = RecipeDatabase.RECIPE_MINOR_BLOOD_PILL
...
func _ready() -> void:
	if multiplayer.is_server():
		...
		if map != null:
			map.register_keyed(map.alchemy_cauldrons, StringName(name), self, "alchemy cauldron")
		return
...
func _on_clicked() -> void:
	...
	if lp == null or not is_instance_valid(lp) or lp._dead or lp.pathfinder == null:
		return
	if lp.is_alchemizing():
		lp.request_stop_alchemy()
		return
...
func _request_start() -> void:
	...
	Client.request_data(
		&"alchemy.start",
		Callable(),
		{"cauldron": String(name), "recipe": String(recipe_id)},
